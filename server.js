...
const SalesHistory = mongoose.models.SalesHistory || mongoose.model('SalesHistory', SalesHistorySchema);
const ProductAdditionHistory = mongoose.models.ProductAdditionHistory || mongoose.model('ProductAdditionHistory', ProductAdditionHistorySchema);
const ProductDeletionHistory = mongoose.models.ProductDeletionHistory || mongoose.model('ProductDeletionHistory', ProductDeletionHistorySchema);
...
app.post('/api/withdrawals', authenticateToken, isAdminOrCompany, async (req, res) => {
  try {
    const { companyName, productCode, productId, productName, quantity, withdrawPrice } = req.body;
    if (!companyName || !productCode || !productName || !quantity || !withdrawPrice) {
      return res.status(400).json({ error: 'กรุณากรอกข้อมูลให้ครบถ้วน' });
    }
    ...
    const withdrawal = {
      ...
      productCode,
      productId: productId || (target ? target.id : null),
      productName: productName || (target ? target.name : ''),
      quantity: qty,
      withdrawPrice: price,
      totalAmount,
