...
let products = [];
let allProducts = []; // เก็บสินค้าทั้งหมดสำหรับ dropdown
let allUsers = []; // เก็บข้อมูล users ทั้งหมดเพื่อหาสาขาเจ้าของสินค้า
let categories = [];
...
function checkAuth() {
    ...
    if (token) {
        ...
        // โหลดข้อมูล users ถ้าเป็น admin หรือ company_user
...
function updateAuthUI() {
    ...
    const addCategoryFromProductsBtn = document.getElementById('addCategoryFromProductsBtn');
    ...
    const withdrawalRequestsNavBtn = document.getElementById('withdrawalRequestsNavBtn');
    const withdrawalRequestsBadge = document.getElementById('withdrawalRequestsBadge');
    ...
    if (currentUser) {
        ...
        // แสดงเมนูเบิกสินค้าสำหรับ Admin และ Company User เท่านั้น (ไม่แสดงสำหรับ User ปกติ)
        if (withdrawalsNavBtn) withdrawalsNavBtn.style.display = (isAdmin || isCompanyUser) ? 'block' : 'none';
        // แสดงเมนูคำขอเบิกสินค้าสำหรับ Admin และ Company User (สาขาที่มีสินค้า)
        if (withdrawalRequestsNavBtn) withdrawalRequestsNavBtn.style.display = (isAdmin || isCompanyUser) ? 'block' : 'none';
        
        // แสดงปุ่มสำหรับ Admin และ Company User
        if (isAdmin || isCompanyUser) {
            addProductBtn.style.display = 'block';
            addCategoryBtn.style.display = 'block';
            if (addCategoryFromProductsBtn) addCategoryFromProductsBtn.style.display = 'block';
            // แสดงเมนูหมวดหมู่ (dropdown)
            if (categoriesDropdown) categoriesDropdown.style.display = 'block';
        } else {
            addProductBtn.style.display = 'none';
            addCategoryBtn.style.display = 'none';
            if (addCategoryFromProductsBtn) addCategoryFromProductsBtn.style.display = 'none';
            if (categoriesDropdown) categoriesDropdown.style.display = 'none';
        }
        
        // แสดงปุ่มเฉพาะ Admin เท่านั้น
        if (isAdmin) {
        ...
        // โหลดคำขอที่รออนุมัติเพื่ออัปเดต badge
        ...
        if (withdrawalRequestsNavBtn) withdrawalRequestsNavBtn.style.display = 'none';
...
function showAddProductModal() {
    ...
    if (branchSelect) {
        branchSelect.innerHTML = '<option value="">เลือกสาขา</option>' + 
            BRANCHES.map(b => `<option value="${b.id}" ${currentUser?.companyId === b.id ? 'selected' : ''}>${b.name}</option>`).join('');
    }
...
function showAddCategoryModal() {
    if (!currentUser || (!isAdmin && !isCompanyUser)) {
        showNotification('กรุณาเข้าสู่ระบบก่อน', 'error');
        return;
    }
...
function showSection(sectionId) {
    ...
    if (sectionId === 'withdrawalRequests') {
        if (isCompanyUser || isAdmin) {
            loadPendingApprovals();
        }
    }
...
function renderCategoriesDropdown() {
    ...
    // ถ้าเป็น Admin ให้มีเมนูเพิ่มหมวดหมู่ใน dropdown ด้วย
    if (isAdmin) {
        items.push(
            `<hr>`,
            `<button class="dropdown-item" onclick="showAddCategoryModal()"><i class="fas fa-plus"></i> เพิ่มหมวดหมู่</button>`
        );
    }
...
async function loadProducts() {
    try {
        ...
        // เก็บสินค้าทั้งหมดไว้สำหรับ dropdown
        allProducts = fetchedProducts;
        
        // ถ้าเป็น company_user ให้กรองเฉพาะสินค้าที่ตัวเองเพิ่ม
        if (isCompanyUser && currentUser) {
            products = fetchedProducts.filter(p => p.createdBy === currentUser.id);
        } else {
...
// โหลดข้อมูล users ทั้งหมดเพื่อหาสาขาเจ้าของสินค้า
            ...
            <div class="product-card">
                ...
                <div class="product-info">
                    ...
                    <div class="product-actions">
                        ...
                        ${(isAdmin || (isCompanyUser && product.createdBy === currentUser?.id)) ? `
...
async function handleAddProduct(e) {
    ...
    if (!branchId) {
        showNotification('กรุณาเลือกสาขา', 'error');
        return;
    }
    
    const selectedBranch = getBranchById(branchId);
    if (!selectedBranch) {
        showNotification('สาขาที่เลือกไม่ถูกต้อง', 'error');
        return;
    }
    ...
    formData.append('companyId', branchId);
    formData.append('companyName', selectedBranch.name);
...
async function showWithdrawalModal() {
    ...
    // โหลดสินค้าทั้งหมดและ users ถ้ายังไม่ได้โหลด
    if (allProducts.length === 0) {
        await loadProducts();
    }
    if (allUsers.length === 0 && (isAdmin || isCompanyUser)) {
        await loadAllUsers();
    }
    
    // เติม dropdown สินค้าจาก allProducts (สินค้าทั้งหมดของทุกสาขา)
    const sel = document.getElementById('withdrawProductId');
    if (sel) {
        // หาข้อมูลสาขาเจ้าของสินค้าแต่ละตัว (ใช้ companyName จากสินค้าโดยตรง หรือหาจาก user)
        const productOptions = (allProducts || []).map(p => {
            // ใช้ companyName จากสินค้าโดยตรง (ถ้ามี) หรือหาจาก user
            let branchName = p.companyName || 'ไม่ระบุสาขา';
            if (!branchName || branchName === 'ไม่ระบุสาขา') {
                const productOwner = p.createdBy ? allUsers.find(u => u.id === p.createdBy) : null;
                branchName = productOwner && productOwner.companyName ? productOwner.companyName : 'ไม่ระบุสาขา';
            }
            const companyId = p.companyId || (p.createdBy ? allUsers.find(u => u.id === p.createdBy)?.companyId : null) || '';
            return `<option value="${p.id}" data-owner-id="${p.createdBy || ''}" data-owner-company="${companyId}" data-owner-name="${branchName}">${p.name} (เหลือ ${p.stock}) - ${branchName}</option>`;
        }).join('');
        sel.innerHTML = `<option value="">ไม่เลือก</option>` + productOptions;
    }
    ...
    // สร้างรหัสสินค้าแบบสุ่มอัตโนมัติให้ช่องรหัสสินค้า
    const codeInput = document.getElementById('withdrawProductCode');
    if (codeInput) {
    ...
    // เติม dropdown สาขา
    const fromLocationSelect = document.getElementById('withdrawFromLocation');
    const toLocationSelect = document.getElementById('withdrawToLocation');
    if (fromLocationSelect) {
        fromLocationSelect.innerHTML = '<option value="">เลือกสาขา</option>' + 
            BRANCHES.map(b => `<option value="${b.name}" ${currentUser?.companyName === b.name ? 'selected' : ''}>${b.name}</option>`).join('');
    }
    if (toLocationSelect) {
        toLocationSelect.innerHTML = '<option value="">เลือกสาขา</option>' + 
            BRANCHES.map(b => `<option value="${b.name}">${b.name}</option>`).join('');
    }

    // ถ้าเลือกสินค้าจากระบบ ให้เติมชื่อสินค้า ราคา และแสดงข้อมูลสาขา
    if (sel) {
        sel.onchange = () => {
            ...
            if (product) {
                const nameInput = document.getElementById('withdrawProductName');
                const priceInput = document.getElementById('withdrawPrice');
                const ownerCompanyId = selectedOption.getAttribute('data-owner-company');
                ...
                // แสดงข้อมูลสาขาเจ้าของสินค้า
                const ownerInfoDiv = document.getElementById('productOwnerInfo');
                if (ownerInfoDiv) {
                    if (ownerName && ownerName !== 'ไม่ระบุสาขา') {
                        ownerInfoDiv.innerHTML = `
                            <div style="margin-top: 0.5rem; padding: 0.75rem; background: var(--secondary-color); border-radius: 8px; border-left: 4px solid var(--primary-color);">
                                <strong><i class="fas fa-store"></i> สาขาเจ้าของสินค้า:</strong> ${ownerName}
                            </div>
                        `;
                        ownerInfoDiv.style.display = 'block';
                    } else {
                        ownerInfoDiv.style.display = 'none';
                    }
                }
                
                // ตั้งค่า fromLocation เป็นสาขาเจ้าของสินค้า
                if (fromLocationSelect && ownerName && ownerName !== 'ไม่ระบุสาขา') {
                    fromLocationSelect.value = ownerName;
                }
            } else {
                const ownerInfoDiv = document.getElementById('productOwnerInfo');
                if (ownerInfoDiv) {
...
async function loadWithdrawals() {
    ...
    try {
        ...
        // โหลดคำขอที่รออนุมัติสำหรับสาขา
        if (isCompanyUser || isAdmin) {
...
// โหลดคำขอเบิกสินค้าที่รออนุมัติ
async function loadPendingApprovals() {
    if (!currentUser || (!isCompanyUser && !isAdmin)) return;
    
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API_URL}/withdrawals/pending-approval`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
// แสดงคำขอเบิกสินค้าที่รออนุมัติ
function renderPendingApprovals(pendingApprovals) {
    ...
    const requestsContainer = document.getElementById('withdrawalRequestsList');
    const badge = document.getElementById('withdrawalRequestsBadge');
    ...
    // สร้าง HTML สำหรับคำขอ
    const html = generatePendingApprovalsHTML(pendingApprovals);
    
    // แสดงในหน้า withdrawals (section เดิม)
    ...
    // แสดงในหน้า withdrawalRequests (หน้าใหม่)
    if (requestsContainer) {
        if (!Array.isArray(pendingApprovals) || pendingApprovals.length === 0) {
            requestsContainer.innerHTML = `
                <div class="empty-state">
                    <i class="fas fa-bell-slash"></i>
                    <h3>ยังไม่มีคำขอเบิกสินค้าที่รออนุมัติ</h3>
                    <p>คำขอเบิกสินค้าจากสาขาอื่นจะแสดงที่นี่</p>
                </div>
            `;
        } else {
...
// สร้าง HTML สำหรับคำขอที่รออนุมัติ
function generatePendingApprovalsHTML(pendingApprovals) {
    return pendingApprovals.map(w => {
        const d = new Date(w.withdrawnAt || w.createdAt);
        
        return `
            <div class="history-card" style="border-left: 4px solid var(--warning-color);">
                <div class="history-header">
                    <div>
                        <h4>${w.productName}</h4>
                    ...
                    <div style="text-align:right;">
                        ...
                        <div style="font-weight: 700; color: var(--primary-color);">
                            ฿${parseFloat(w.totalAmount || 0).toFixed(2)}
                        </div>
                    </div>
                </div>
                <div class="history-items">
                    ...
                    ${w.fromLocation && w.toLocation ? `
                        <div style="margin-top: 0.5rem;">
                            ...
                            <strong>จาก:</strong> ${w.fromLocation} 
                            <i class="fas fa-arrow-right" style="margin: 0 0.5rem; color: var(--text-light);"></i> 
                            <strong>ไป:</strong> ${w.toLocation}
                        </div>
                    ` : ''}
                    <div style="color: var(--text-light); margin-top: 0.25rem;">
                        <i class="fas fa-user"></i> ${w.withdrawnByUsername || ''}
                    </div>
...
async function approveWithdrawal(withdrawalId) {
    if (!confirm('คุณต้องการอนุมัติคำขอเบิกสินค้านี้หรือไม่?')) {
        return;
    }
    
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API_URL}/withdrawals/${withdrawalId}/approve`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        if (response.ok) {
            showNotification('อนุมัติคำขอเบิกสินค้าสำเร็จ', 'success');
            await loadPendingApprovals();
            await loadWithdrawals();
            await loadProducts(); // รีเฟรชสินค้าเพื่ออัปเดต stock
            
            // ถ้าอยู่ในหน้า withdrawalRequests ให้รีเฟรช
            const currentSection = document.querySelector('.section.active');
            if (currentSection && currentSection.id === 'withdrawalRequests') {
                await loadPendingApprovals();
            }
        } else {
            showNotification(data.error || 'อนุมัติคำขอไม่สำเร็จ', 'error');
        }
    } catch (error) {
...
async function rejectWithdrawal(withdrawalId) {
    if (!confirm('คุณต้องการปฏิเสธคำขอเบิกสินค้านี้หรือไม่?')) {
        return;
    }
    
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API_URL}/withdrawals/${withdrawalId}/reject`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        if (response.ok) {
            showNotification('ปฏิเสธคำขอเบิกสินค้าสำเร็จ', 'success');
            await loadPendingApprovals();
            await loadWithdrawals();
            ...
            showNotification(data.error || 'ปฏิเสธคำขอไม่สำเร็จ', 'error');
        }
    } catch (error) {
...
function renderWithdrawals() {
    ...
    container.innerHTML = withdrawals.map(w => {
                    ...
                    : status === 'shipping'
                        ...
                        : status === 'delivered'
                            ...
                            : status === 'received'
                                ? 'ได้รับสินค้าแล้ว'
                                : 'รอดำเนินการ';
        const statusClass = status === 'pending_approval'
            ? 'badge-warning'
            : status === 'approved'
                ? 'badge-success'
                : status === 'rejected'
        ...
        if (isAdmin) {
            ...
            const allowedStatuses = ['pending', 'approved', 'shipping', 'delivered'];
            actionsHtml = `
                <div style="margin-top: 0.5rem; display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center;">
                    <select id="${selectId}" class="admin-select">
                        ${allowedStatuses.map(s => `<option value="${s}" ${status === s ? 'selected' : ''}>${s === 'pending' ? 'รอดำเนินการ' : s === 'approved' ? 'อนุมัติแล้ว' : s === 'shipping' ? 'สินค้ากำลังจัดส่ง' : 'สินค้าถึงที่หมายแล้ว'}</option>`).join('')}
                    </select>
        ...
        return `
            <div class="history-card">
                ...
                <div class="history-items">
                    ...
                    ` : ''}
                    ${w.requestedFromCompanyName ? `
                        <div style="margin-top: 0.5rem; color: var(--warning-color);">
                            <i class="fas fa-exclamation-triangle"></i> 
                            <strong>ขอเบิกจาก:</strong> ${w.requestedFromCompanyName}
                        </div>
                    ` : ''}
...
async function handleWithdrawal(e) {
    ...
    try {
        ...
        const payload = {
            ...
            fromLocation: document.getElementById('withdrawFromLocation').value,
            toLocation: document.getElementById('withdrawToLocation').value
        };
        ...
        if (response.ok) {
            ...
            // ถ้าเบิกสินค้าจากสาขาอื่น ให้โหลดคำขอที่รออนุมัติเพื่ออัปเดต badge
            if (isCompanyUser || isAdmin) {
                await loadPendingApprovals();
            }
        } else {
                        ...
                        ${isAdmin && order.status !== 'cancelled' && order.status !== 'completed' ? `
                            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center;">
                                <select id="${selectId}" class="admin-select">
                                    ...
                                    <option value="unpaid" ${order.status === 'unpaid' ? 'selected' : ''}>ค้างชำระ</option>
                                    <option value="awaiting_delivery" ${order.status === 'awaiting_delivery' ? 'selected' : ''}>รอการส่งมอบ</option>
                                ...
                                <button class="btn btn-primary btn-sm" onclick="updateOrderStatusFromSelect('${order.id}', '${selectId}')">
...
function showUpdateOrderStatusModal(orderId) {
    // ฟังก์ชันนี้ไม่ถูกใช้แล้ว (แทนที่ด้วย dropdown ใน renderOrders)
    console.log('showUpdateOrderStatusModal is deprecated. Use dropdown instead.');
}
...
function updateOrderStatusFromSelect(orderId, selectId) {
    const select = document.getElementById(selectId);
    if (!select) return;
    const status = select.value;
